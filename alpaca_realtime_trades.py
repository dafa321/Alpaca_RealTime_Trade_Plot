...
class AlpacaStream:
    ...
    def __init__(self, symbol):
        ...
        self.fig, self.axs = plt.subplots(2, 1, sharex=True, gridspec_kw={'height_ratios': [3, 1]}, figsize=(14, 8))
        self.xdata, self.ydata, self.vdata = [], [], []
        self.ln_price, = self.axs[0].plot([], [], 'r-', label=self.symbol)
        self.axs[0].set_ylabel('Trade Price')
        self.axs[0].grid(True, which='both', color='lightgrey', linestyle='--')
        self.axs[0].legend(loc='best')
        self.axs[1].bar(self.xdata, self.vdata, width=0.0001, align='center', color='b', label='Volume')
        self.axs[1].set_xlabel('Time')
        self.axs[1].set_ylabel('Trade Volume')
        self.axs[1].grid(True, which='both', color='lightgrey', linestyle='--')
    ...
    def update_plot(self, new_x, new_y, new_v):
        ...
        self.axs[1].grid(True, which='both', color='lightgrey', linestyle='--')
        self.fig.suptitle(f'Real-time {self.symbol} Trades - Price: {new_y} Volume: {new_v}', fontsize=12)
        plt.draw()
        plt.pause(0.01)
    ...
    async def handle_trades(self, websocket):
        ...
        while True:
            ...
            for msg in msg_json:
                if msg['T'] == 't':
                    print(msg)
                    new_x = md.date2num(self.rfc3339_to_local_datetime(msg['t']))
                    new_y = msg['p']
                    new_v = msg['s']
...
parser = argparse.ArgumentParser(description='Stream trades for a specific symbol.')
parser.add_argument('--symbol', '-s', type=str, required=True, help='The stock symbol to stream trades for.')
